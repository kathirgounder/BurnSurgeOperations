import {
    ...
    vec2,
    vec3
  } from 'https://cdn.jsdelivr.net/npm/gl-matrix@2.8.1/+esm';
  ...
  const CustomLayerView2D = BaseLayerViewGL2D.createSubclass({
    ...
    attach: function () {
      ...
      const fragmentSource = `
            ...
            uniform float u_core_radius;   // half length of the central square
            uniform float u_arm_width;     // half width of each arm
            ...
            /* ── helpers ─────────────────────────────── */
            float hashp(vec2 p){ return fract(sin(dot(p,vec2(127.1,311.7)))*43758.5453); }


            /* distance to an axis‑aligned rectangle, centre at (0,0) */
            float sdRect(vec2 p, vec2 h) {                  // h = half‑size (hx, hy)
                vec2 d = abs(p) - h;
                return length(max(d, 0.0)) + min(max(d.x, d.y), 0.0);
            }

            /* distance to a cross: union of a vertical + horizontal bar */
            float sdCross(vec2 p, float L, float t) {       // L = half arm‑length
                float dH = sdRect(p, vec2(L, t));             // horizontal bar
                float dV = sdRect(p, vec2(t, L));             // vertical bar
                return min(dH, dV);                           // union → min()
            }


        void main(){
            vec2 p = v_offset * u_size_px;
            float d = sdCross(p, u_core_radius, u_arm_width);  // NEW


            /* hard core */
            float core = 1.0 - smoothstep(-1.0, +1.0, d);

            /* soft halo (still circular for contrast) */
            float r = length(p);
            float glow = exp(-pow(max(r - u_core_radius,0.0) /
                                    (u_glow_radius - u_core_radius), 2.0));

            /* pulse + sparkle exactly like the dot layer */
            float pulse = 1.0 + 0.20 *
                            sin(u_current_time*6.28318*u_pulse_freq);

            float rnd   = hashp(floor(gl_FragCoord.xy/128.0));
            float spark = 1.0 + u_spark_ampl *
                            sin(u_current_time*6.28318*u_spark_freq + rnd*6.28318);
  ...
  `;
      ...
      this.uArmWidth  = gl.getUniformLocation(this.program, "u_arm_width");
    ...
    render: function (renderParameters) {
      ...
      const armPx = this.layer.armWidth * this.layer.sizePx;
      gl.uniform1f(this.uArmWidth,  armPx);
  ...
  export const CrossLayer = GraphicsLayer.createSubclass({
    properties:{
      ...
      sizePx:     { value: 70.0 },
      armWidth : { value: 0.18 }   // as *fraction* of sizePx
    },
